...
//emu_printf("Video\n");
	_video = new Video();
	_paf->_shadowLayer = _video->_shadowLayer;
	_cheats = cheats;
#ifdef DEMO
	_playDemo = false;
#endif
...
//emu_printf("shadow screen j %d/%d\n", j, size);
...
#else
	const int rounds = ((g_system->getTimeStamp() & 15) + 1);
	_rnd.initTable(rounds);
#endif
